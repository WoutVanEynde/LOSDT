    ...
    with open(pdb_file_path, "r") as in_f, open(receptor_output_path, "w") as out_f:
        for line in pdb_delresname.run(in_f, ligand_resname):
            ...
            if line.startswith('CONECT'):
                atoms = [int(line[i:i+5].strip()) for i in range(6, len(line.strip()), 5) if line[i:i+5].strip().isdigit()]
...
def predict_admet_properties(smiles: str, output_csv_path: Path) -> None:
...
def align_molecules(template_molecule_path: Path, 
                   ...
                   derivatives_to_align_path: Path) -> None:
    """Run molecular alignment using direct import."""
    try:
        ensure_templates_in_path()
        from align_molecules_isotope_methodology_parallel_with_hydrogens import align_molecules_main
        #from align_molecules_parallel import align_molecules_main
...
def create_complexes(receptor_pdb: Path, 
                    aligned_molecules_dir: Path, 
                    complexes_dir: Path) -> None:
...
def plot_radials(results_csv_path: Path, 
    ...
    try:
        ensure_templates_in_path()
        from plot_radials_parallel import create_radial_plots_main
...
def process_input_data(smiles_input: Optional[str], 
    ...
    # Final validation
    if not smiles_input or not is_valid_smiles(smiles_input):
        raise ValueError("Invalid, missing SMILES or wrong ligand name in PDB. Please provide valid SMILES or change ligand name in PDB file.")
    
    return smiles_input, pdb_file_path

def run_molecular_pipeline(smiles_string: str, 
                          pdb_file_path: Optional[Path], 
                          session_folder: Path) -> Dict:
    """
    ...
    predict_admet_properties(smiles_string, results_csv_path)
    ...
    if pdb_file_path:
        ...
        align_molecules(ligand_sdf, aligned_dir, results_csv_path)
...
def display_results(session_folder: Path, smiles_string: str, has_pdb: bool):
    ...
    with tab1:
        ...
        if radial_dir.exists():
            ...
            if plot_files:
                current_plot = plot_files[st.slider(label="Select radial plot", min_value=0, max_value=len(plot_files)- 1)]          
                ...
                st.dataframe(display_df.style.map(color_lipinski, subset=['Lipinski']), column_config={'structure_image': st.column_config.ImageColumn("Structure",width="medium", pinned=True),
                                                ...
                                                'hydrogen_bond_acceptors': st.column_config.NumberColumn("HBA",format="%.0f"),
                                                'hydrogen_bond_donors': st.column_config.NumberColumn("HBD",format="%.0f"),
                                                'Lipinski': st.column_config.NumberColumn("Lipinski",format="%.0f"),
                                                'QED': st.column_config.NumberColumn("QED",format="%.2f"),
                                                'stereo_centers': st.column_config.NumberColumn("Stereo centers",format="%.0f"),
                                                'tpsa': st.column_config.NumberColumn("TPSA",format="%.2f"),
                                                'AMES': st.column_config.ProgressColumn("Mutagenicity (AMES)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'BBB_Martins': st.column_config.ProgressColumn("BBB permeability (Martins)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'Bioavailability_Ma': st.column_config.ProgressColumn("Bioavailability (Ma)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP1A2_Veith': st.column_config.ProgressColumn("CYP1A2 inhibition (Veith)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP2C19_Veith': st.column_config.ProgressColumn("CYP2C19 inhibition (Veith)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP2C9_Substrate_CarbonMangels': st.column_config.ProgressColumn("CYP2C9 inhibition (CarbonMangels)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP2C9_Veith': st.column_config.ProgressColumn("CYP2C9 inhibition (Veith)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP2D6_Substrate_CarbonMangels': st.column_config.ProgressColumn("CYP2D6 inhibition (CarbonMangels)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP2D6_Veith': st.column_config.ProgressColumn("CYP2D6 inhibition (Veith)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP3A4_Substrate_CarbonMangels': st.column_config.ProgressColumn("CYP3A4 inhibition (CarbonMangels)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'CYP3A4_Veith': st.column_config.ProgressColumn("CYP3A4 inhibition (Veith)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'Carcinogens_Lagunin': st.column_config.ProgressColumn("Carcinogic (Lagunin)",format="%.2f",min_value=0.0,max_value=1.0),
                                                'ClinTox': st.column_config.ProgressColumn("Clinical trial toxicity",format="%.2f",min_value=0.0,max_value=1.0),
                                                'DILI': st.column_config.ProgressColumn("Drug Induced Liver Injury",format="%.2f",min_value=0.0,max_value=1.0),
                                                'HIA_Hou': st.column_config.ProgressColumn("Human Intestinal Absorption",format="%.2f",min_value=0.0,max_value=1.0),
                                                'PAMPA_NCATS': st.column_config.ProgressColumn("PAMPA Permeability",format="%.2f",min_value=0.0,max_value=1.0),
                                                'Pgp_Broccatelli': st.column_config.ProgressColumn("P-glycoprotein inhibition",format="%.2f",min_value=0.0,max_value=1.0),
                                                'Skin_Reaction': st.column_config.ProgressColumn("Skin reaction",format="%.2f",min_value=0.0,max_value=1.0),
                                                'hERG': st.column_config.ProgressColumn("hERG inhibition",format="%.2f",min_value=0.0,max_value=1.0),
                                                'Caco2_Wang': st.column_config.NumberColumn("Cell Effective Permeability (Caco-2 Wang)",format="%.2f"),
                                                'Clearance_Hepatocyte_AZ': st.column_config.NumberColumn("Clearance hepatocyte (AstraZeneca)",format="%.2f"),
                                                'Clearance_Microsome_AZ': st.column_config.NumberColumn("Clearance microsome (AstraZeneca)",format="%.2f"),
                                                'Half_Life_Obach': st.column_config.NumberColumn("Half life (Obach)",format="%.2f"),
                                                'HydrationFreeEnergy_FreeSolv': st.column_config.NumberColumn("Hydration Free Energy",format="%.2f"),
                                                'LD50_Zhu': st.column_config.NumberColumn("LD50 (Zhu)",format="%.2f"),
                                                'Lipophilicity_AstraZeneca': st.column_config.NumberColumn("Lipophilicity (AstraZeneca)",format="%.2f"),
                                                ...
                                                'Bioavailability_Ma_drugbank_approved_percentile': st.column_config.ProgressColumn("Bioavailability (Ma) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP1A2_Veith_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP1A2 inhibition (Veith) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP2C19_Veith_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP2C19 inhibition (Veith) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP2C9_Substrate_CarbonMangels_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP2C9 inhibition (CarbonMangels) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP2C9_Veith_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP2C9 inhibition (Veith) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP2D6_Substrate_CarbonMangels_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP2D6 inhibition (CarbonMangels) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP2D6_Veith_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP2D6 inhibition (Veith) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP3A4_Substrate_CarbonMangels_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP3A4 inhibition (CarbonMangels) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'CYP3A4_Veith_drugbank_approved_percentile': st.column_config.ProgressColumn("CYP3A4 inhibition (Veith) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'Carcinogens_Lagunin_drugbank_approved_percentile': st.column_config.ProgressColumn("Carcinogic (Lagunin) drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'ClinTox_drugbank_approved_percentile': st.column_config.ProgressColumn("Clinical trial toxicity drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'DILI_drugbank_approved_percentile': st.column_config.ProgressColumn("Drug Induced Liver Injury drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'HIA_Hou_drugbank_approved_percentile': st.column_config.ProgressColumn("Human Intestinal Absorption drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'PAMPA_NCATS_drugbank_approved_percentile': st.column_config.ProgressColumn("PAMPA Permeability drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'Pgp_Broccatelli_drugbank_approved_percentile': st.column_config.ProgressColumn("P-glycoprotein inhibition drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'Skin_Reaction_drugbank_approved_percentile': st.column_config.ProgressColumn("Skin reaction drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'hERG_drugbank_approved_percentile': st.column_config.ProgressColumn("hERG inhibition drugbank approved percentile",format="%.2f",min_value=0.0,max_value=100.0),
                                                'Caco2_Wang_drugbank_approved_percentile': st.column_config.ProgressColumn("Cell Effective Permeability (Caco-2 Wang) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'Clearance_Hepatocyte_AZ_drugbank_approved_percentile': st.column_config.ProgressColumn("Clearance hepatocyte (AstraZeneca) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'Clearance_Microsome_AZ_drugbank_approved_percentile': st.column_config.ProgressColumn("Clearance microsome (AstraZeneca) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'Half_Life_Obach_drugbank_approved_percentile': st.column_config.ProgressColumn("Half life (Obach) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'HydrationFreeEnergy_FreeSolv_drugbank_approved_percentile': st.column_config.ProgressColumn("Hydration Free Energy drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'LD50_Zhu_drugbank_approved_percentile': st.column_config.ProgressColumn("LD50 (Zhu) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'Lipophilicity_AstraZeneca_drugbank_approved_percentile': st.column_config.ProgressColumn("Lipophilicity (AstraZeneca) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'PPBR_AZ_drugbank_approved_percentile': st.column_config.ProgressColumn("Plasma Protein Binding Rate (AstraZeneca) drugbank approved percentile",format="%.2f",max_value=100.0),
                                                'Solubility_AqSolDB_drugbank_approved_percentile': st.column_config.ProgressColumn("Solubility (AqSolDB) drugbank approved percentile",format="%.2f",max_value=100.0),
        ...
        if has_pdb:
            ...
            if complexes_dir.exists():
                ...
                if complex_files:
                    ...
                    st.info("No complex structures available.")
            else:
                st.info("No 3D structures generated.")
        else:
    ...
    with tab3:        
        ...
        if has_pdb:
            zip_path = session_folder / Config.ZIP_FILENAME
            if zip_path.exists():
                st.markdown("Download the structural data; Volumetric shape and ESP similarity score can be found in the last column of the ADMET results file.")
            ...
            st.warning("Structural data is not available because no PDB file was uploaded.")
...
def main():
    ...
    if not st.session_state.results_ready:
        ...
        with input_tab2:
            ...
            smile_code = st.text_input(f"Enter or draw:")
        ...
        with col2:
            if st.button("**ANALYZE MOLECULE**", type="primary", width="stretch"):
                ...
                if not smiles_input and pdb_file is None:
                    st.error("Please provide either a SMILES string or upload a PDB file.")
                elif smiles_input and not is_valid_smiles(smiles_input) and pdb_file is None:
                    st.error("Invalid SMILES notation. Please provide a valid SMILES or upload a PDB file.")
                else:
                    # Start processing
