...
if getattr(sys, "frozen", False):
    ...
    os.environ["LD_LIBRARY_PATH"] = (
        bundle_dir + os.pathsep + os.environ.get("LD_LIBRARY_PATH", "")
    )
...
def _transfer_coordinates_via_mcs(
    original_mol: Chem.Mol, new_mol: Chem.Mol
) -> Chem.Mol:
    """Transfer coordinates from original to new molecule using MCS."""
    mcs = rdFMCS.FindMCS(
        [original_mol, new_mol],
...
) -> Tuple[Optional[Chem.Mol], List[int], List[int]]:
...
def remove_1xx_terminal_atoms_paired(mol, tpl, mol_atoms, tpl_atoms):
    ...
    while changed:
        ...
        for i, (mol_idx, tpl_idx) in enumerate(zip(mol_atoms, tpl_atoms)):
            ...
            if 100 <= tpl_isotope:
                tpl_neighbors_in_mcs = sum(
                    1
                    for neighbor in tpl_atom.GetNeighbors()
        ...
        if atom.IsInRing():
            for neighbor in atom.GetNeighbors():
                ...
                if neighbor.IsInRing():
                    ...
                    shares_ring = any(
                        atom.GetIdx() in ring and neighbor.GetIdx() in ring
...
def find_multiple_mcs_alignments(
    molecule: Chem.Mol, template: Chem.Mol
    ...
    if mol_match1 and tpl_match1:
        ...
        mol_match1, tpl_match1 = remove_1xx_terminal_atoms_paired(
        ...
        if mol_match2 and tpl_match2:
            ...
            mol_match2, tpl_match2 = remove_1xx_terminal_atoms_paired(
                molecule, template, mol_match2, tpl_match2
            )
            ...
            if mol_match2 and tpl_match2:
                ...
                if mol_match3 and tpl_match3:
                    mol_match3, tpl_match3 = remove_1xx_terminal_atoms_paired(
                        molecule, template, mol_match3, tpl_match3
                    )
...
def apply_coordinate_alignment(
    molecule: Chem.Mol,
    template: Chem.Mol,
...
def optimize_molecule_geometry(molecule: Chem.Mol, fixed_atoms: List[int]) -> None:
    ...
    for i in range(10):
        try:
            ...
            ff_copy = AllChem.MMFFGetMoleculeForceField(
                mol_copy, AllChem.MMFFGetMoleculeProperties(mol_copy)
            )
            for idx in fixed_atoms:
...
def _progressive_constraint_optimization(
    molecule: Chem.Mol, constrained_atom_indices: list[int]
) -> None:
    ...
    for k in force_constants:
        ff = AllChem.MMFFGetMoleculeForceField(
            molecule, AllChem.MMFFGetMoleculeProperties(molecule)
        )
...
def shepherd_scoring(
    aligned_molecule: Chem.Mol, template: Chem.Mol
    ...
    try:
        ...
        ref_charges = charges_from_single_point_conformer_with_xtb(
            template, charge=formal_charge_template
        )
        fit_charges = charges_from_single_point_conformer_with_xtb(
            aligned_molecule, charge=formal_charge_aligned
        )
        ...
        # Perform alignment
        mp.align_with_vol_esp(
            ALPHA(mp.num_surf_points),
    ...
    try:
        ...
        if status == -1:
            return (
                ...
                index,
                None,
                f"Failed to generate 3D conformer for: {smiles}",
            )
        ...
        return (
            ...
            index,
            aligned_mol_block,
            f"Successfully aligned molecule {index}",
        )
...
def process_single_shepherd_scoring(args):
    ...
    try:
        ...
        return (
            ...
            scored_mol_block,
            vol_esp_score,
            f"Successfully scored molecule {index}",
        )
...
def align_molecules_parallel(
    smiles_list: List[str], template: Chem.Mol, n_processes: int = None
    ...
    # Prepare arguments
    args_list = [
        (i, smiles, template_mol_block) for i, smiles in enumerate(smiles_list)
    ]
    ...
    logger.info(
        f"Successfully aligned {successful_alignments}/{len(smiles_list)} molecules"
    )
    logger.info("=" * 60)
...
def score_molecules_parallel(
    aligned_molecules: List[Chem.Mol],
    ...
    """
    logger.info(
        f"Starting parallel shepherd scoring of {len(aligned_molecules)} molecules"
    )
    ...
    scores = [
        1.0 if i == 0 else 0.0 for i in range(len(aligned_molecules))
    ]  # Reference gets score 1.0
    ...
    if args_list:  # Only process if there are molecules to score
        ...
        end_time = time.time()
        logger.info(
            f"Parallel scoring completed in {end_time - start_time:.2f} seconds"
        )
        logger.info(
            f"Successfully scored {successful_scores}/{len(args_list)} molecules"
        )

    return scored_molecules, scores


def align_molecules_serial(
    smiles_list: List[str], template: Chem.Mol
) -> Tuple[List[Chem.Mol], List[float]]:
            ...
            if i > 0:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        shepherd_scoring, aligned_mol, template_for_scoring
                    )
                    try:
                        shepherd_aligned, vol_esp_score = future.result(
                            timeout=Config.TIMEOUT
                        )
                        aligned_molecules[i] = shepherd_aligned
                        scores.append(vol_esp_score)
                        logger.info(
                            f"Successfully processed molecule {i + 1}/{len(smiles_list)}"
                        )
                    except concurrent.futures.TimeoutError:
                        logger.error(
                            f"Timeout: Failed to score molecule {i} ({smiles}) within {Config.TIMEOUT} seconds"
                        )
                        scores.append(0.0)
            else:
...
def align_molecules_main(
    ...
    try:
        ...
        if method == "serial":
            aligned_molecules_list, scores = align_molecules_serial(
                smiles_list, template
            )
        elif method == "parallel":
            # Two-phase parallel processing
            aligned_molecules_list, error_messages = align_molecules_parallel(
                smiles_list, template, processes
            )

            # Parallel scoring
            scored_molecules, scores = score_molecules_parallel(
                aligned_molecules_list, smiles_list, template, processes
            )
        ...
        saved_count = 0
        for i, aligned_mol in enumerate(
            aligned_molecules_list[1:], start=1
        ):  # Skip first molecule
        ...
        if len(scores) == len(derivatives_df):
            ...
            logger.warning(
                f"Score count mismatch: {len(scores)} scores vs {len(derivatives_df)} molecules"
            )
