        ...
        if atom.IsInRing():
            for neighbor in atom.GetNeighbors():
                ...
                if neighbor.IsInRing():
                    ...
                    shares_ring = any(
                            atom.GetIdx() in ring and neighbor.GetIdx() in ring for ring in ring_info.AtomRings()
                    )
                    if not shares_ring:
                        # Count number of rings the neighbor belongs to
                        neighbor_ring_count = sum(1 for ring in ring_info.AtomRings() 
...
def align_molecules_main(
    ...
    try:
        ...
        if method == 'serial':
            ...
            # Use first successful alignment as reference for scoring
            reference_mol = None
            for mol in aligned_molecules_list:
                if mol is not None:
                    reference_mol = mol
                    break
            
            if reference_mol is None:
                raise RuntimeError("No successful alignments found!")
            
            # Parallel scoring
            scored_molecules, scores = score_molecules_parallel(aligned_molecules_list, smiles_list, reference_mol, processes)
