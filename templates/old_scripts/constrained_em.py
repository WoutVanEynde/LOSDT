...
def extract_ligands_from_pdb(pdb_path: Path, ligand_residue_names: Optional[List[str]] = None) -> Dict[str, List[Molecule]]:
    """
        ...
        for idx, (residue, chain_id, res_id) in enumerate(residue_list):
            ...
            try:
                ...
                for atom in mol.GetAtoms():
                    ...
                    n_double = sum(1 for b in atom.GetBonds()
...
def run_constrained_em_single(
    ...
    try:
        ...
        ligand_molecules = extract_ligands_from_pdb(pdb_path, ligand_residue_names)
...
def constrained_em_main(
    ...
    """
    Run constrained energy minimization on multiple protein-ligand complexes sequentially.
    """
    ...
    for i, pdb_path in enumerate(pdb_files, 1):
        logger.info("=" * 60)
        logger.info(f"[{i}/{len(pdb_files)}] Processing: {pdb_path.name}")
        logger.info("=" * 60)
        result = run_constrained_em_single(
            pdb_path=pdb_path,
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--pdb-dir', required=True, type=str,
                       help='Directory containing complex PDB files')
    parser.add_argument('--output-dir', required=True, type=str,
                       help='Directory for output files')
    parser.add_argument('--ligand', type=str, nargs='+', default=None,
                       help='Ligand residue name (e.g. LIG, NAD). Auto-detect if not provided.')
    parser.add_argument('--restraint-radius', type=float, default=DEFAULT_RESTRAINT_RADIUS,
    ...
    parser.add_argument('--minimization-steps', type=int, default=DEFAULT_MINIMIZATION_STEPS,
