...
def predict_admet_for_molecules(molecules_df: pd.DataFrame, model: ADMETModel) -> pd.DataFrame:
...
def predict_admet_for_reactions(input_smiles: str, 
                              ...
                              output_csv_path: Path) -> pd.DataFrame:
    ...
    # Predict ADMET properties
    results_with_admet = predict_admet_for_molecules(results_df, model)
    
    # Create input molecule entry
    input_data = create_input_molecule_entry(input_smiles, input_mol_properties, model)
